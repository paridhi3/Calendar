...
export default function Day({
  ...
  return (
    <div
      className={`relative border rounded cursor-pointer group transition-all
        ${today
          ? "bg-gradient-to-br from-yellow-300 to-white text-white border-yellow-500 shadow-lg shadow-pink-300"
          : isCurrentMonth
          ? "bg-white/60"
    ...
    >
      <div className="flex flex-col h-full p-2">
        ...
        {/* Events list */}
        <div className="flex-1 mt-2 overflow-y-auto space-y-1 scrollbar-thin scrollbar-thumb-indigo-400">
          {user &&
            events.map((ev) => (
              <EventCard key={ev.id} event={ev} onSaved={onUpdated} user={user} />
            ))}

          {/* Add new event button inside events list */}
          {events.length > 0 && (
            <div
              className="flex items-center justify-center mt-2 cursor-pointer text-indigo-600 hover:text-indigo-800"
              title="Add new event"
        ...
        {events.length === 0 && (
          <div
            className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 text-white font-bold text-xs sm:text-sm p-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded text-center"
            onClick={handleClick}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => {
